...
{
#if !WITH_EDITOR
	char* ImportHookModuleName = PYTHON_IMPORT_HOOK_MODULE_NAME;
...
{
#if !WITH_EDITOR
	FString PakFilePath = FPaths::ConvertRelativePathToFull(FPaths::ProjectContentDir() / FString(TEXT(PAK_PACKAGE_FILE_RELATIVE_PATH)));
	...
	if (PakFile.IsValid())
	{
		...
		{
			FString PackageFileNameToSearch = ScriptPath / FString(TFullName) / FString(TEXT("__init__.pyc"));
			FString ModuleFileNameToSearch = ScriptPath / FString(TFullName) + FString(TEXT(".pyc"));
